...
Тут я должен приостановиться и объяснить что я называю плагином. В yii2 предусмотрены расширения ([yii2 extensions](http://www.yiiframework.com/doc-2.0/guide-structure-extensions.html)) и они дают возможность организовывать переиспользуемый код и подключать его к проекту композером. Но мало-мальски сложное расширение нуждается в конфигурации. И тут фреймворк не помогает. У создателя расширения есть два варианта:
...
Одно из *"базовых приложений"*, которые мы развиваем — HiSite [hiqdev/hisite](https://github.com/hiqdev/hisite) — это основа для типичного сайта, как `yii2-app-basic,` только сделанная как плагин, что даёт все преимущества переиспользования кода над копипастингом:
...
Шаблон *"корня"* для проекта на HiSite здесь — [hiqdev/hisite-template](https://github.com/hiqdev/hisite-template).

Иерархия зависимостей выглядит так:

- *"корень"* — [hiqdev/hisite-template](https://github.com/hiqdev/hisite-template);

плагин темы — [hiqdev/yii2-theme-flat](https://github.com/hiqdev/yii2-theme-flat);

библиотека тем — [hiqdev/yii2-thememanager](https://github.com/hiqdev/yii2-thememanager);

- базовый проект — [hiqdev/hisite](https://github.com/hiqdev/hisite);

фреймворк — [yiisoft/yii2](https://github.com/yiisoft/yii2).

В [README](https://github.com/hiqdev/hisite-template) корня описано как поднять проект у себя — `composer create-project` плюс настройка конфигурации. Благодаря реализации тем как плагинов и библиотеке тем [hiqdev/yii2-thememanager](https://github.com/hiqdev/yii2-thememanager) в `composer.json` корня можно поменять `yii2-theme-flat` на `yii2-theme-original` запустить `composer update` и сайт переоденется в новую тему. Вот так просто.

Ещё один реальный рабочий проект, подходящий в качестве примера, сделанный, используя этот подход и полностью доступный на GitHub — [Asset Packagist](https://asset-packagist.org) — packagist-совместимый репозиторий, который позволяет устанавливать Bower и NPM пакеты как нативные composer пакеты.

Иерархия зависимостей выглядит так:

- *"корень"* — [hiqdev/asset-packagist.dev](https://github.com/hiqdev/asset-packagist.dev);
...
Мы используем описанный подход около года, впечатления самые положительные — волосы стали мягкие и шелковистые: разделяем и властвуем, клепаем плагины легко и непринуждённо, [100+](https://hiqdev.com/pages/packages) и останавливаться не собираемся, нужен новый функционал — делаем новый плагин.
